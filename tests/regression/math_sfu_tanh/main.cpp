...
// System-level tanh contract (math-sfu spec): relative error <= 2^-13 vs a
// double-precision reference, same adjudication domain as the unit oracle
// (tests/cocotb/math_sfu_unit/math_rtl_model.py): NaN in -> qNaN out,
// tanh(+-inf) = +-1, |x| < 2^-7 pass-through (bit-exact), |x| >= 5.25
// saturates to +-1 (within the bound).
...
// Odd-symmetry commitment: every finite directed value appears as an
// (x, -x) pair.
static const uint32_t SPECIALS[NUM_SPECIALS] = {
    ...
    0x807FFFFF,  // negative largest subnormal
    0x7F800000,  // +inf               (inf)
    0xFF800000,  // -inf               (inf)
    0x7FC00000,  // qNaN               (nan)
    0xFFC00000,  // -qNaN              (nan)
    0x7F800001,  // NaN payload        (nan)
    0x3D000000,  // 2^-7 bypass boundary
    0x3CFFFFFE,  // just below 2^-7
    0x3F000000,  // 0.5
    0x3F800000,  // 1.0
    0xBF800000,  // -1.0               (odd_symmetry_pair)
    0x40000000,  // 2.0
    0xC0000000,  // -2.0               (odd_symmetry_pair)
    0x40480000,  // 3.125 (seg2 center)
    0xC0480000,  // -3.125             (odd_symmetry_pair)
    0x40800000,  // 4.0
    0xC0800000,  // -4.0               (odd_symmetry_pair)
    0x40A00000,  // 5.0
    0xC0A00000,  // -5.0               (odd_symmetry_pair)
    0x40A80000,  // 5.25 saturation boundary
    0xC0A80000,  // -5.25              (odd_symmetry_pair)
    0x40A7FFFF,  // just below 5.25
    0xC0A7FFFF,  // just below -5.25
    0x42C80000,  // 100.0              (saturation_tail)
    0xC2C80000,  // -100.0             (saturation_tail)
};
...
static bool check_tanh(uint32_t x_bits, uint32_t got_bits, uint32_t idx, int errors) {
    float xf = bits_to_f32(x_bits);
    float got = bits_to_f32(got_bits);
    if (std::isnan(xf)) {
        if (!std::isnan(got)) {
            if (errors < 5)
                printf("TANH nan [%u]: x=0x%08x got=0x%08x expected NaN\n", idx, x_bits, got_bits);
            return false;
        }
        return true;
    }
    ...
    if (xf == std::numeric_limits<float>::infinity()) {
        ...
        ref = std::tanh((double)xf);
    }
    bool ok;
    if (std::fabs(ref) < FTZ_LIMIT) {
        // x = +-0 or subnormal: hardware pass-through is bit-exact.
        ok = (got == xf);
    } else {
    ...
    if (!ok && errors < 5)
        printf("TANH mismatch [%u]: x=0x%08x got=0x%08x ref=%.9e\n", idx, x_bits, got_bits, ref);
    return ok;
}
...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < total; ++i) {
        if (!check_tanh(h_src[i], h_dst[i], i, errors))
            ++errors;
    }
