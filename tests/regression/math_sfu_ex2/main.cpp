...
// System-level 2^x contract (math-sfu spec): relative error <= 2^-13 vs a
...
// x >= 128 -> +Inf, x < -126 -> FTZ +0, subnormal outputs exempt.
...
static const uint32_t SPECIALS[NUM_SPECIALS] = {
    ...
    0x00800000,  // smallest normal 2^-126
    0x7F800000,  // +inf              (inf)
    0xFF800000,  // -inf              (inf)
    0x7FC00000,  // qNaN              (nan)
    0x7F800001,  // NaN payload       (nan)
    0x43000000,  // 128.0             (overflow_sat -> +inf)
    0x42FFFFFF,  // ~127.99999        (overflow edge, finite)
    0xC3000000,  // -128.0            (underflow_sat -> 0)
    0xC2FEFFFF,  // ~-128             (underflow_sat -> 0)
    0xC2FC0000,  // -126.0            (domain_edge, FTZ boundary, normal out)
    0xC2FC0001,  // just below -126   (domain_edge, FTZ -> 0)
    0x3F000000,  // 0.5               (domain_edge, split boundary)
    0xBF000000,  // -0.5              (domain_edge)
    0x3F800000,  // 1.0
    0x40490FDB,  // pi
    0xC0490FDB,  // -pi
};
...
// Host double-precision reference with the spec's saturation/FTZ semantics.
// Returns false on a bound violation, printing the first few offenders.
static bool check_ex2(uint32_t x_bits, uint32_t got_bits, uint32_t idx, int errors) {
    float xf = bits_to_f32(x_bits);
    float got = bits_to_f32(got_bits);
    if (std::isnan(xf)) {
        if (!std::isnan(got)) {
            if (errors < 5)
                printf("EX2 nan [%u]: x=0x%08x got=0x%08x expected NaN\n", idx, x_bits, got_bits);
            return false;
        }
        return true;
    }
    ...
    if (xf == std::numeric_limits<float>::infinity()) {
        ref = std::numeric_limits<double>::infinity();
    } else if (xf == -std::numeric_limits<float>::infinity()) {
        ref = 0.0;
    } else if ((double)xf >= 128.0) {
        ref = std::numeric_limits<double>::infinity();
    } else if ((double)xf < -126.0) {
        ref = 0.0;
    } else {
        ref = std::pow(2.0, (double)xf);
    }
    bool ok;
    if (std::isinf(ref)) {
    ...
    if (!ok && errors < 5)
        printf("EX2 mismatch [%u]: x=0x%08x got=0x%08x ref=%.9e\n", idx, x_bits, got_bits, ref);
    return ok;
}
...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < total; ++i) {
        if (!check_ex2(h_src[i], h_dst[i], i, errors))
            ++errors;
    }
