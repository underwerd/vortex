...
// Host reference: bf16 operands widened to f32, exact f32 sum, then one RNE
// step to bf16. Since the f32 mantissa (p1=24) exceeds the bf16 mantissa
// (p2=8) by more than 2 bits, double rounding cannot differ from the exact
// sum rounded once (Figueroa's theorem), so this matches the RTL's
...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < total; ++i) {
        ...
        uint16_t r_lo = bf16_add(a_lo, b_lo);
        uint16_t r_hi = bf16_add(a_hi, b_hi);
    ...
    for (uint32_t i = 0; i < total; ++i) {
        if (h_dst[i] != ref[i]) {
            if (errors < 5)
                printf("ADD mismatch [%u]: got=0x%08x exp=0x%08x (a=0x%08x b=0x%08x)\n",
