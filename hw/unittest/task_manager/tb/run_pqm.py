#!/usr/bin/env python3
# cocotb runner for the VX_tm_pqm standalone unit test (small-module-first).
import os
...
SOURCES = [
    ...
    os.path.join(VX, "hw/unittest/task_manager/VX_tm_pqm.sv"),
]
BUILD_ARGS = [
    "-Wno-fatal", "--assert",
...
def main():
    ...
    runner.build(
        sources=SOURCES,
        hdl_toplevel="VX_tm_pqm",
        build_dir=os.path.join(WORK, "build_pqm"),
        build_args=BUILD_ARGS,
        always=True,
    )
    runner.test(
        hdl_toplevel="VX_tm_pqm",
        test_module="test_tm_pqm",
        build_dir=os.path.join(WORK, "build_pqm"),
        results_xml=os.path.join(WORK, "results_pqm.xml"),
    )
