#!/usr/bin/env python3
# cocotb runner for the VX_tm_tsm standalone unit test (small-module-first).
import os
...
SOURCES = [
    ...
    os.path.join(VX, "hw/unittest/task_manager/VX_tm_tsm.sv"),
]
BUILD_ARGS = [
    "-Wno-fatal", "--assert",
...
def main():
    ...
    runner.build(
        sources=SOURCES,
        hdl_toplevel="VX_tm_tsm",
        build_dir=os.path.join(WORK, "build_tsm"),
        build_args=BUILD_ARGS,
        always=True,
    )
    runner.test(
        hdl_toplevel="VX_tm_tsm",
        test_module="test_tm_tsm",
        build_dir=os.path.join(WORK, "build_tsm"),
        results_xml=os.path.join(WORK, "results_tsm.xml"),
    )
